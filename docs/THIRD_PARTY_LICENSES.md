...
| JarzRover colored-ball model and labels | `colored_ball_yolov5.tflite`; `colored_balls.txt` | Added in commit `27aa942`. The JARzLabs team photographed and annotated the balls. The supplied Roboflow evidence identifies public workspace/project slugs, 89 source images, 3 classes, a 71/9/9 split, Auto-Orient, Fit with black edges at 512×512, and a YOLOv5 PyTorch download path. See the [model card](models/COLORED_BALL_MODEL.md). | **Partially resolved.** Preserve the completed version/export record, verify CC BY 4.0 applies to the distributed weights, and document actual augmentation, architecture/version, export settings, metrics, and attribution. If those cannot be established, omit the binary from the first signed public release. |
