...
- Not run: Android/iOS device installation, simulator UI, website/email installation, physical rover/controller behavior, signed APK/IPA creation, or TestFlight/Ad Hoc provisioning. The reviewed source was subsequently pushed to both private and public `jarz-development`; model/AAR notice gaps documented in `docs/THIRD_PARTY_LICENSES.md` remain binary-release gates.

## Roboflow colored-ball provenance evidence — 2026-09-20

- Reviewed the supplied Roboflow Browse PDF, Create New Version/Augmentation PDF, screenshots, and the linked Roboflow YOLOv5 training notebook. The PDFs show workspace/project slugs `aarav-patel-k0rgk/openbot-colored-ball-detector`, 89 source images, 3 classes, 0 unannotated images, and the original ball photographs with bounding boxes.
